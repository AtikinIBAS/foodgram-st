...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def subscribe(self, request, pk=None):
        try:
            ...
            return Response({"detail": "Неверный ID автора"}, status=status.HTTP_400_BAD_REQUEST)
        user = request.user
    ...
    def unsubscribe(self, request, pk=None):
        ...
        user = request.user
        author = get_object_or_404(User, pk=pk)
        follow = Follow.objects.filter(follower=user, following=author).first()
        if not follow:
    ...
    def subscriptions(self, request):
        ...
        serializer = FollowSerializer(result_page, many=True, context={"request": request})
    ...
    def set_password(self, request):
        ...
        if not current_password or not new_password:
            return Response({"detail": "Необходимо указать current_password и new_password."},
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            ...
            if is_in_shopping_cart == '1':
                queryset = queryset.filter(shopping_carts__user=user).distinct()
    ...
    def favorite(self, request, pk=None):
        ...
        if request.method == 'POST':
            ...
            if favorite:
                favorite.delete()
                return Response(status=status.HTTP_204_NO_CONTENT)
            return Response({"detail": "Не в избранном"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def shopping_cart(self, request, pk=None):
        ...
        if request.method == 'POST':
            ...
            if shopping_cart:
                ...
                return Response(status=status.HTTP_204_NO_CONTENT)
            return Response({"detail": "Рецепт не в корзине"}, status=status.HTTP_400_BAD_REQUEST)

    @action(detail=False, methods=['get'], permission_classes=[permissions.IsAuthenticated])
    def shopping_cart_ingredients(self, request):
        user = request.user
        shopping_cart = ShoppingCart.objects.filter(user=user)
        ingredients = Ingredient.objects.filter(recipe__in=shopping_cart.values('recipe'))
    ...
    def get_short_link(self, request, pk=None):
        recipe = self.get_object()
        short_link = request.build_absolute_uri(reverse('short-link', args=[recipe.short_uuid]))
